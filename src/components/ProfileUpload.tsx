...
export function ProfileUpload(): ReactElement {
  ...
  const simulateUpload = (fileName: string): void => {
    setCvName(fileName);
    setStatus('scanning');
    ...
    <div className="mx-auto w-full max-w-2xl">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-8">
          {status === 'idle' ? (
            ...
            >
              ...
              <input id="cv-upload" type="file" className="hidden" onChange={handleBrowse} />
            </label>
          ) : null}
          ...
          {status === 'scanning' ? (
            <div className="flex flex-col items-center gap-4 rounded-2xl border border-indigo-100 bg-indigo-50/60 py-16 text-center">
              ...
              <p className="flex items-center gap-2 text-sm text-indigo-900">
                ...
                Identifying biomedical techniques & certifications
              </p>
            </div>
          ) : null}
            ...
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-6">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-slate-900">{cvName}</h3>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                {[
                  { label: 'Skills identified', value: '24' },
                  { label: 'Education level', value: 'BSc Biomedical Science' },
                  { label: 'Clinical experience', value: 'NHS placement' },
                ].map((item) => (
                  <div key={item.label} className="rounded-2xl border border-white bg-white/80 p-4 shadow-sm">
        ...
        <div className="flex items-start gap-3 border-t border-slate-100 bg-slate-50 px-6 py-5 text-sm text-slate-500">
          ...
          <span>
            <strong>Privacy note:</strong> Your CV is stored securely and only used to match opportunities. Nothing is shared without your permission.
          </span>
        </div>
      </div>
    </div>
